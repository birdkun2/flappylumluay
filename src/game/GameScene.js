...
import { speedForScore, nextGap, overlapsObstacle, creditPass, readBest, saveBest } from './rules.js';
...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.input.keyboard.on('keydown-SPACE', event => { if (!event.repeat) this.handleInput(); });
  ...
  startRun() {
    ...
    this.player.setPosition(C.PLAYER_X, C.PLAYER_Y).setAngle(0).setAlpha(1);
    this.paused = false; this.pauseText?.destroy();
    this.state = STATE.PLAYING;
    this.flap();
    this.announce('Go! Tap, click, or press Space to flap.');
  }
  flap() { this.velocity = C.FLAP_VELOCITY; this.player.play('flight', true); }
  spawnPair() {
  ...
  die() {
    ...
    this.best = Math.max(this.best, this.score); saveBest(this.storage, this.best);
